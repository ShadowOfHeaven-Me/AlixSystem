...
/*    private static File getPluginFile(String fileName) {
    ...
    private enum Validation {
        ...
        //if "firewall: true" is the line, "#firewall: true" should also be valid, and the line should be considered present
...
/*    public static void updateFile(File existingFile, File newestFile, char spliterator) { I'll add it in the future
    ...
    private static void ensureUpdated(File existingFile, File newestFile, char spliterator, List<Validation> validate) {
        ...
        try {
            if (!existingLines.equals(newestLines))
                AlixFileManager.write(existingFile, newestLines);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
