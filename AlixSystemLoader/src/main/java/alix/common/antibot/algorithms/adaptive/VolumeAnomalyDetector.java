...
final class VolumeAnomalyDetector {
    ...
    private final double slack, elevatedAt, attackAt;
    private State state = State.NORMAL;

    VolumeAnomalyDetector(double alpha, int windowSize, double slack, double elevatedAt, double attackAt) {
    ...
    State onBucket(double current) {
        ...
        if (cusum > attackAt || z > 8) state = State.ATTACK;
