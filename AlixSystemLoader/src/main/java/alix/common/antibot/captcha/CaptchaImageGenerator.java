...
/*    public static boolean filter(Color color) {//for map captcha
    ...
    public static byte[] imageToBytes(BufferedImage image) {
        ...
        for (int i = 0; i < pixels.length; ++i) {
            byte color = matchColor(new Color(pixels[i], true));
            //if (color >= green - diff && color <= green + diff) color = 0;
            result[i] = color; //noColorIfGreen(color);
            /*if (!debugged && i == 0) {
                AlixCommonMain.logError("BYTE COLOR: " + result[0]);
                debugged = true;
            }*/
        }
    ...
    /*private static byte noColorIfGreen(byte color) {
        switch (color) {
    ...
    public static byte matchColor(Color color) {
        ...
        if (color.getAlpha() < 128) {
            ...
            for (int i = 4; i < _MPColors.mapColors.length; ++i) {
                double distance = getDistance(color, _MPColors.mapColors[i]);
            ...
            return (byte) (index < 128 ? index : -255 + index);
        }
    }
