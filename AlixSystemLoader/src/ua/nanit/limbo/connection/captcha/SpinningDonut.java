...
import alix.libs.com.github.retrooper.packetevents.protocol.component.ComponentTypes;
...
public final class SpinningDonut implements Runnable {
    ...
    public void run() {
        ...
        for (int i = 0; i < mapsTotal; i++) {
            ...
            this.connection.writePacket(this.createMap(mapId++, subImage));
        }
    ...
    private void createEntityMap(Vector3d pos, BufferedImage image, int entityId, int mapId, List<PacketOut> list) {
    ...
    private PacketPlayOutMap createMap(int mapId, BufferedImage image) {
        byte[] bytes = CaptchaImageGenerator.imageToBytes(image);
        int offsetX = 0;
        int offsetZ = 0;
        WrapperPlayServerMapData map = new WrapperPlayServerMapData(mapId, (byte) 0, false, false, null, 128, 128, offsetX, offsetZ, bytes);
