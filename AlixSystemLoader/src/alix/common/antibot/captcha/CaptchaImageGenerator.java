...
public final class CaptchaImageGenerator {
    ...
    public static final int MIN_RGB = 25;
    public static final int RBG_BOUND = 224 - MIN_RGB;//224 instead of 256 in order to prevent the generation of whitish-colored characters, which would be impossible to read
    ...
    //From MapPalette
    private static final Color[] colors = _MPColors.mapColors;
    public static final List<Color> PARTICLE_COLOR_LIST = new ArrayList<>(Arrays.asList(_PAColors.particleColors));
    ...
    public static BufferedImage generateCaptchaImage(String captcha, int maxRotation, boolean drawLines, boolean antialiasing, Supplier<Color> randomColor) {
        return generator.generate(captcha, maxRotation, drawLines, antialiasing, randomColor);
    }

    public static byte[] generatePixelsToDraw(String captcha, int maxRotation, boolean drawLines, boolean antialiasing) {
        return imageToBytes(generateCaptchaImage(captcha, maxRotation, drawLines, antialiasing, CaptchaImageGenerator::getMapColor));
    }

    private static Color nextDifferentColor(Color differentFrom, Supplier<Color> randomColor) {
        Color color;
        do {
            color = randomColor.get();
        } while (differentFrom != null && isSimilar(color, differentFrom));
        return color;
    }
    ...
    public static Color getParticleColor() {
        return PARTICLE_COLOR_LIST.get(random.nextInt(PARTICLE_COLOR_LIST.size()));
    }
    ...
    private interface ImageGenerator {
        ...
        BufferedImage generate(String captcha, int maxRotation, boolean drawLines, boolean antialiasing, Supplier<Color> randomColor);
    ...
    private static final class DefaultGenerator implements ImageGenerator {
        ...
        @Override
        public BufferedImage generate(String captcha, int maxRotation, boolean drawLines, boolean antialiasing, Supplier<Color> randomColor) {
            //image with alpha
...
/*            try {
            ...
            while (lines-- != 0) {
                graphics.setColor(randomColor.get());
            ...
            for (int i = 0; i != chars.length; i++) {
                ...
                Color color = nextDifferentColor(lastColor, randomColor);
                lastColor = color;
...
/*    private static final class SomeGen implements ImageGenerator {
    ...
    private static final class _PAColors {
        ...
        private static final Color[] particleColors = new Color[]{
                c(255, 215, 0),     // Gold
                c(255, 165, 0),     // Orange
                c(255, 0, 0),       // Red
