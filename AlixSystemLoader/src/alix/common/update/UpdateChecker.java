...
/*        if(currentFirstWord.equals(newestFirstWord) && newestVersionSplit.length != 1 && currentVersionSplit.length != 1) {//these are dev builds, we'll
        ...
        for (int i = 0; i < newestVer.length; i++) {
            ...
            if (newestVersionParsed < currentVersionParsed) return false;
            if (newestVersionParsed > currentVersionParsed) return true;
        }
        return false;
    }

    private static void getVersion(Consumer<String> consumer) {
        try (final InputStream inputStream = new URL("https://api.spigotmc.org/legacy/update.php?resource=109144").openStream();
             final Scanner scanner = new Scanner(inputStream)) {
            if (scanner.hasNext()) {
                consumer.accept(scanner.next());
            }
        } catch (IOException e) {
            BukkitAlixMain.instance.getLogger().warning("Could not get the newest version - Check your internet connection!");
        }
    }
