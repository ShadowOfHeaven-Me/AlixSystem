...
public final class Captcha3d {

    private static double A = 0, B = 0; // Angles for rotation
    ...
    public static void spinningDonut(ClientConnection connection) {
        List<PacketOut> list = createSpinningDonut(createSpinningDonutImg());

        list.forEach(connection::writePacket);
        connection.flush();
    }

    private static List<PacketOut> createSpinningDonut(BufferedImage image) {
        List<PacketOut> list = new ArrayList<>();
        //the rendering pos in the very middle
        Vector3d middle = new Vector3d(0, 64, 2);

        assert image.getWidth() == image.getHeight();
        //how many maps are in each dimension of the square
        int mapsInOneDim = image.getWidth() / 128;
        //the total amount of maps in the square
        int mapsTotal = mapsInOneDim * mapsInOneDim;
        ...
        for (int i = 0; i < mapsTotal; i++) {
            ...
            createMap(pos, subImage, entityId++, mapId++, list);
        }
        return list;
    }

    private static void createMap(Vector3d pos, BufferedImage image, int entityId, int mapId, List<PacketOut> list) {
        ItemStack item = ItemStack.builder().type(ItemTypes.FILLED_MAP).build();

        WrapperPlayServerSpawnEntity entity = new WrapperPlayServerSpawnEntity(entityId, Optional.of(UUID.randomUUID()), EntityTypes.ITEM_FRAME, pos, 0, 0, 0, BlockFace.NORTH.getFaceValue(), Optional.of(Vector3d.zero()));
        //https://wiki.vg/Entity_metadata#Item_Frame
        WrapperPlayServerEntityMetadata metadata = new WrapperPlayServerEntityMetadata(entityId, Collections.singletonList(new EntityData(8, EntityDataTypes.ITEMSTACK, item)));

        byte[] bytes = CaptchaImageGenerator.imageToBytes(image);
        int offsetX = 0;
        int offsetZ = 0;

        WrapperPlayServerMapData map = new WrapperPlayServerMapData(mapId, (byte) 0, false, true, null, 128, 128, offsetX, offsetZ, bytes);

        list.add(new PacketPlayOutSpawnEntity(entity));
        list.add(new PacketPlayOutEntityMetadata(metadata));
        list.add(new PacketPlayOutMap(map));
    }

    private static BufferedImage createSpinningDonutImg() {
        int width = 1024;
        int height = 1024;
