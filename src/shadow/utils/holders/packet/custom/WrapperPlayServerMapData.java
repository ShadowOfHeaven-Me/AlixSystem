...
public class WrapperPlayServerMapData extends PacketWrapper<WrapperPlayServerMapData> {
    ...
    public WrapperPlayServerMapData(int mapId, byte scale, boolean locked, @Nullable List<MapIcon> icons) {
    ...
    public WrapperPlayServerMapData(int mapId, byte scale, boolean locked, @Nullable List<MapIcon> icons, int columns, int rows, int x, int z, @NotNull byte[] bytes) {
        super(PacketType.Play.Server.MAP_DATA);
        this.mapId = mapId;
        this.scale = scale;
        this.locked = locked;
    ...
    public void read() {
        ...
        if (hasIcons)//length is already read within
            this.icons = this.readList(w -> new MapIcon(MapIconType.ofId(this.readVarInt()), this.readByte(), this.readByte(), this.readByte(), this.readBoolean() ? this.readComponent() : null));
    ...
    public void write() {
        ...
        if (hasIcons) {
            this.writeList(this.icons, (tis, icon) -> {
