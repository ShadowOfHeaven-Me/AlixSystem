...
import static shadow.utils.objects.savable.data.password.builders.PasswordGui.*;

public final class SimplePinBuilder implements PasswordBuilder {
    ...
    public boolean select(int slot) {//returns whether the login action should be performed
        ...
        return performAction(slot, player);
    }
...
/*        if (slot == DIGIT_0) return '0';
    ...
    private boolean append(char c, Player player) {
        ...
        if (!pinAutoConfirm && pin.length() == 4) player.playSound(this.currentLocation, Sound.BLOCK_NOTE_BLOCK_HARP, 1.0f, 1.0f);
