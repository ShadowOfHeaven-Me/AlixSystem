...
public final class VerifiedPacketProcessor implements PacketProcessor {
    ...
    private final Consumer<VerifiedUser> onFirstPlayPacket;
    private boolean firstPlayPacketReceived;
    ...
    private VerifiedPacketProcessor(VerifiedUser user, Consumer<VerifiedUser> onFirstPlayPacket) {
        this.user = user;
        this.onFirstPlayPacket = onFirstPlayPacket;
    ...
    public void onPacketReceive(PacketPlayReceiveEvent event) {
        if (!this.firstPlayPacketReceived) {
            this.firstPlayPacketReceived = true;
            this.onFirstPlayPacket.accept(this.user);
        }
...
/*    public void stop() {
    ...
    public static VerifiedPacketProcessor getProcessor(VerifiedUser user, Consumer<VerifiedUser> onFirstPlayPacket) {
        return new VerifiedPacketProcessor(user, onFirstPlayPacket); //PacketBlocker.serverboundNameVersion ? new VerifiedPacketProcessor(user, handler) : null;
    }
}